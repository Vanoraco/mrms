    ...
    useEffect(() => {
        if (sessionStorage.getItem("authUser")) {
            ...
            setUserName(process.env.REACT_APP_DEFAULTAUTH === "fake" ? 
                obj.username || (user && user.name) || obj.data?.name || "Admin" :
                process.env.REACT_APP_DEFAULTAUTH === "firebase" ? obj.email || "Admin" : "Admin"
            );
        }
    }, [userName, user]);
        ...
        <React.Fragment>
            <Dropdown isOpen={isProfileDropdown} toggle={toggleProfileDropdown} className="ms-sm-3 header-item topbar-user">
                ...
                <DropdownMenu className="dropdown-menu-end">
                    ...
                    <DropdownItem className='p-0'>
                        <Link to={process.env.PUBLIC_URL + "/logout"} onClick={logOut} className="dropdown-item">
                            <i
